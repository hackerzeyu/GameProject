...
int Channel::recvProc()
{
    ...
    if (n < 0)
    {
        ...
        {
            Erro("Unexpected errno:EAGAIN,LT should not appear this!");
            return -1;
        }
        if (errno == EINTR)
        {
            Erro("Unexpeceted errno:EINTR,interrupted by signal!");
            return -1;
        }
        if (errno == ECONNRESET)
        {
        ...
        {
            Erro("recv error,errmsg=%s", strerror(errno));
        }
        // 属于有问题的连接,直接回收!
        TcpServer::getInstance()->recycle(this);
        return -1;
...
{
    PkgHeader pkgHeader;
    memcpy(&pkgHeader, m_header, _HEAD_SIZE_);
    uint16_t msgLen = ntohs(pkgHeader.len);
    // 消息长度比包头小,鉴定为恶意包
    if (msgLen < _HEAD_SIZE_)
    {
        Erro("find invalid packet,throw away!!!");
        TcpServer::getInstance()->recycle(this);
        return -1;
    }
    // 消息长度>29000,鉴定为恶意包
    ...
    {
        Erro("find too long packet,throw away!!!");
        // TODO
        TcpServer::getInstance()->recycle(this);
        return -1;
    }
...
int Channel::handleSend(void *args)
{
    ...
    {
        // 一般不可能
        Erro("write buffer is full,impossible!!!");
        return -1;
    }
    if (sendSize > 0 && sendSize == channel->m_sendLen || sendSize == -2)
    {
        ...
        {
            Erro("handleSend epollOper failed!!!");
            return -1;
        }
    }
    // 消息发送完毕
...
{
    m_redis = new Room;
    m_redis->connectToRedis();
}

void Channel::closeRedis()
{
    if (m_redis)
        delete m_redis;
}