...
int TcpServer::epollProcessEv(int timer)
{
    ...
    if (n == -1)
    {
        ...
        {
            Erro("Unknown error in epoll_wait:%s", strerror(errno));
            // 异常
            return -1;
        }
    }
    else if (n == 0)
    {
        ...
        Erro("Unexpected return 0 in epoll_wait,error=%s!", strerror(errno));
        return -1;
    }
    for (int i = 0; i < n; i++)
    {
...
int TcpServer::epollOper(int fd, int event, int opt, int other, Channel *channel)
{
    ...
    if (ret == -1)
    {
        Erro("epoll_oper_ctl failed,errmsg=%s", strerror(errno));
    }
    return ret;
}
