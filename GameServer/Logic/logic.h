...
public:
    ...
    bool _HandleCreateRoom(Channel *, MsgHeader &, char *body, uint16_t bodyLength); // 创建房间
    bool _HandleJoinRoom(Channel *, MsgHeader &, char *body, uint16_t bodyLength);   // 添加房间
    bool _HandleSearchRoom(Channel *, MsgHeader &, char *body, uint16_t bodyLength); // 搜索房间
    bool _HandlePlay(Channel *, MsgHeader &, char *body, uint16_t bodyLength);       // 处理下棋动作
    bool _HandleEnd(Channel *, MsgHeader &, char *body, uint16_t bodyLength);        // 处理结束事件
